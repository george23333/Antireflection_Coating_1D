...
class ForwardPINNConfig:
    ...
    # Domain size for visualization only
    ...
    # Collocation points only inside coating
    n_f_coating: int = 400
...
def compute_optimal_layer(
    ...
    """
    Optimal single-layer AR coating at normal incidence (lossless):
        n2 = sqrt(n1*n3), d = lambda2 / 4 = c/(4*f0*n2)
    """
    c0 = 299_792_458.0
...
def solve_single_layer_analytic(
    ...
    """
    Solve amplitudes for:
      region 1: E1 = exp(-jk1 x) + r exp(+jk1 x)
      region 2: E2 = A exp(-jk2 x) + B exp(+jk2 x)
      region 3: E3 = t exp(-jk3 (x-d))
    with E and dE/dx continuity at x=0 and x=d.
    """
    c0 = 299_792_458.0
    k0 = 2.0 * np.pi * f / c0
...
def analytic_field(
    ...
    E[m1] = Ei * (np.exp(-1j * k1 * x[m1]) + r * np.exp(1j * k1 * x[m1]))
    E[m2] = Ei * (A * np.exp(-1j * k2 * x[m2]) + B * np.exp(1j * k2 * x[m2]))
    E[m3] = Ei * t * np.exp(-1j * k3 * (x[m3] - d))
    return E
...
def coating_pde_loss(
    ...
    """
    x_hat = x / l_ref  =>  d²E/dx_hat² + (2*pi*n2)^2 E = 0
    """
    x_hat = x_phys / l_ref
...
def coating_field_from_nn(
    model: FCNComplex,
    x_phys: torch.Tensor,
    l_ref: float,
) -> torch.Tensor:
    x_hat = x_phys / l_ref
    return model.field_complex(x_hat)
...
def compute_losses(
    ...
    ei = torch.complex(torch.tensor(cfg.Ei, dtype=torch.float32, device=DEVICE), torch.tensor(0.0, dtype=torch.float32, device=DEVICE))
...
def evaluate_piecewise_field(
    ...
    _ = n1, n3  # only kept for readability / possible extensions

    j = torch.complex(torch.tensor(0.0, device=x_grid.device), torch.tensor(1.0, device=x_grid.device))
    ei = torch.complex(torch.tensor(cfg.Ei, dtype=torch.float32, device=x_grid.device), torch.tensor(0.0, dtype=torch.float32, device=x_grid.device))
    ...
    with torch.inference_mode():
        if torch.any(mask1):
            x1 = x[mask1]
            e_out[mask1] = ei * (
                torch.exp(-j * k1 * x1) + r * torch.exp(j * k1 * x1)
            )
...
def run_forward_pinn(config: ForwardPINNConfig) -> dict[str, Any]:
    cfg = config
    c0 = 299_792_458.0
    ...
    loss_pde_hist: list[float] = []
    loss_if_hist: list[float] = []
    loss_energy_hist: list[float] = []
    ...
    for ep in range(1, cfg.epochs + 1):
        losses = compute_losses(model_coating, scat, cfg, collocation, phys)
